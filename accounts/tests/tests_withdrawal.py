...
class WithdrawalTestCase(APITestCase):
    ...
    def test_withdrawal_success(self):
        ...
        response = self.client.post(f'/accounts/{self.eight_percent_account1.id}/withdrawal/', data=data, content_type='application/json')

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.json()['balance'], 0)


    def test_withdrawal_failed_duto_no_login(self):
        data = """{
            'amount'      : '40000',
            'description' : '해킹'
        }"""

        response = self.client.post(f'/accounts/{self.eight_percent_account2.id}/withdrawal/', data=data, content_type='application/json')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_withdrawal_failed_duto_diffent_user_account(self):
    ...
    def test_withdrawal_failed_duto_invalid_json_data(self):
        self.client.force_authenticate(self.eight_percent_user)
        data = """{
            "amount"      : "이만원",
            "description" :  "간식"
    ...
    def test_withdrawal_failed_duto_invalid_json_key(self):
        self.client.force_authenticate(self.eight_percent_user)
        data = """{
            "금액"        : "20000",
            "description" :  "간식"
    ...
    def test_withdrawal_failed_duto_no_body(self):
        self.client.force_authenticate(self.eight_percent_user)
        data = """{
        }"""
